    ...
    const clientOptions = {
      ...
      middleware: {
        provideOnTypeFormattingEdits: (document2, position, ch, options, token, next) => {
          const line = document2.getText(import_coc3.Range.create(position.line, 0, position.line, position.character));
          if (!line.trim().length)
            return;
          if (ch === "\n")
            ch = "";
...
var ReloadFeature = class {
  ...
  initialize(caps) {
    ...
    const fileWatcher = import_coc5.workspace.createFileSystemWatcher("**/{compile_commands.json,compile_flags.txt}");
  ...
  async reload(url) {
    ...
    if (notification) {
      ...
      import_coc5.window.showMessage(msg);
    }
    ...
    this.activate();
    if (notification)
      import_coc5.window.showMessage(`clangd has reloaded`);
  }
};
...
var MemoryUsageFeature = class {
  ...
  initialize(capabilities) {
    if ("memoryUsageProvider" in capabilities) {
      this.ctx.subscriptions.push(import_coc6.commands.registerCommand("clangd.memoryUsage", async () => {
...
var ASTRequestType = new import_coc7.RequestType("textDocument/ast");
var ASTFeature = class {
  ...
  initialize(capabilities) {
    if ("astProvider" in capabilities && typeof import_coc7.window["createTreeView"] === "function") {
      const adapter = new TreeAdapter();
      const tree = import_coc7.window.createTreeView("clangd.AST", { treeDataProvider: adapter });
      this.ctx.subscriptions.push(tree, adapter.onDidChangeTreeData(() => {
        if (adapter.hasRoot()) {
          tree.reveal(null);
        }
      }), import_coc7.commands.registerCommand("clangd.ast", async () => {
        if (!this.ctx.client)
          return;
        let range = null;
        const { document: document2, position } = await import_coc7.workspace.getCurrentState();
        const mode = await import_coc7.workspace.nvim.call("visualmode");
        if (mode)
          range = await import_coc7.workspace.getSelectedRange(mode, import_coc7.workspace.getDocument(document2.uri));
        if (!range)
          range = import_coc7.Range.create(position, position);
        const params = {
        ...
        if (!item) {
          import_coc7.window.showInformationMessage("No AST node at selection");
          return;
        }
        const winid = await import_coc7.workspace.nvim.eval("win_getid()");
        adapter.setRoot(item, import_coc7.Uri.parse(document2.uri), winid);
        tree.show();
      }));
    }
  }
...
var TreeAdapter = class {
  constructor() {
    this._onDidChangeTreeData = new import_coc7.Emitter();
  ...
  getTreeItem(node) {
    const item = new import_coc7.TreeItem(describe(node.role, node.kind));
    if (node.children && node.children.length > 0) {
      item.collapsibleState = import_coc7.TreeItemCollapsibleState.Expanded;
    }
